...
def discover_setup_packages(
    ...
    for package in products:
        ...
        packages[name] = info
        logger.debug("Found setup package: %s %s %s", name, info["version"], info["dir"])

    return packages


def find_table_file(root_project_dir: str | Path) -> Path:
    ...
    if table_path is None:
        raise RuntimeError(f"Could not find the EUPS table file for {root_project_dir}")
    return table_path
...
def list_packages_in_eups_table(table_text: str) -> list[str]:
    ...
    -------
    names
        List of package names that are required by the EUPS table file.
    """
    logger = logging.getLogger(__name__)
...
class Package:
    ...
    doxygen_conf_path: Path | None = field(default=None)
    ...
    doxygen_conf_in_path: Path | None = field(default=None)
    """Path to the ``doxygen.conf.in`` file, indicating Doxygen documentation
    should be generated.
    """


def find_package_docs(package_dir: str | Path, skipped_names: list[str] | None = None) -> Package:
    ...
    if not modules_yaml_path.is_file():
        raise NoPackageDocsError(r"Manifest YAML not found: {modules_yaml_path}")
    ...
    if "statics" in manifest_data:
        for static_dirname in manifest_data["statics"]:
            ...
            if not full_static_dir.is_dir():
                logger.warning("_static doc dir not found: %s", full_static_dir)
                continue

            # Make a relative path to `_static` that's used as the
            # link source in the root docproject's _static/ directory
            relative_static_dir = str(full_static_dir.relative_to(package.doc_dir / "_static"))
...
class NoPackageDocsError(Exception):

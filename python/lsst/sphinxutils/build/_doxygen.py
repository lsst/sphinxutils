...
class DoxygenConfiguration:
    ...
    inputs: list[Path] = field(default_factory=list, metadata={"doxygen_tag": "INPUT"})
    """Individual paths to be input into the doxygen build.
    """

    image_paths: list[Path] = field(default_factory=list, metadata={"doxygen_tag": "IMAGE_PATH"})
    """Paths that contain images."""

    excludes: list[Path] = field(default_factory=list, metadata={"doxygen_tag": "EXCLUDE"})
    """File or directory paths to be excluded from the inputs.
    """

    recursive: bool = field(default=True, metadata={"doxygen_tag": "RECURSIVE"})
    ...
    exclude_symbols: list[str] = field(default_factory=list, metadata={"doxygen_tag": "EXCLUDE_SYMBOLS"})
    ...
    project_brief: str = field(default="C++ API Reference", metadata={"doxygen_tag": "PROJECT_BRIEF"})
    ...
    generate_html: bool = field(default=True, metadata={"doxygen_tag": "GENERATE_HTML"})
    """Whether or not to generate HTML output.
    """

    generate_latex: bool = field(default=False, metadata={"doxygen_tag": "GENERATE_LATEX"})
    ...
    tagfiles: list[str] = field(
        default_factory=lambda: [
            f"{get_cpp_reference_tagfile_path().resolve()}=http://en.cppreference.com/w/"
        ],
    ...
    generate_xml: bool = field(default=True, metadata={"doxygen_tag": "GENERATE_XML"})
    ...
    use_mathjax: bool = field(default=True, metadata={"doxygen_tag": "USE_MATHJAX"})
    """Enable MathJax to render math, rather than LaTeX.
    """

    mathjax_format: str = field(default="HTML-CSS", metadata={"doxygen_tag": "MATHJAX_FORMAT"})
    ...
    xml_programlisting: bool = field(default=False, metadata={"doxygen_tag": "XML_PROGRAMLISTING"})
    """Whether to include the program listing in the XML output.
    """

    create_subdirs: bool = field(default=False, metadata={"doxygen_tag": "CREATE_SUBDIRS"})
    """Whether Doxygen should create subdirectories.

    This should be NO for breathe/exhale to work.
    """

    full_path_names: bool = field(default=True, metadata={"doxygen_tag": "FULL_PATH_NAMES"})
    """Doxygen keeps the full path of each file, rather than stripping it.
    """

    strip_from_path: list[Path] = field(default_factory=list, metadata={"doxygen_tag": "STRIP_FROM_PATH"})
    ...
    warn_if_undocumented: bool = field(default=True, metadata={"doxygen_tag": "WARN_IF_UNDOCUMENTED"})
    """Warn about undocumented members.

    If EXTRACT_ALL is set to YES then this flag will automatically be disabled.
    """

    warn_if_doc_error: bool = field(default=True, metadata={"doxygen_tag": "WARN_IF_DOC_ERROR"})
    """Warn about potential errors in the documentation."""

    warn_no_paramdoc: bool = field(default=False, metadata={"doxygen_tag": "WARN_NO_PARAMDOC"})
    ...
    warn_as_error: bool = field(default=False, metadata={"doxygen_tag": "WARN_AS_ERROR"})
    ...
    def _render_bool(self, lines: list[str], tag_name: str, value: bool) -> None:
        if value:
            line = f"{tag_name} = YES"
        else:
            line = f"{tag_name} = NO"
    ...
    def _render_path_list(self, lines: list[str], tag_name: str, value: list[Path]) -> None:
        sep = "="
        for p in value:
    ...
    def _render_str_list(self, lines: list[str], tag_name: str, value: list[str]) -> None:
        sep = "="
        for item in value:
    ...
    def __iadd__(self, other: DoxygenConfiguration) -> DoxygenConfiguration:
    ...
    @classmethod
    def from_doxygen_conf(cls, conf_text: str, root_dir: Path) -> DoxygenConfiguration:
        ...
        conf_text = "\n".join([_ for _ in conf_text.split("\n") if not _COMMENT_PATTERN.match(_)])
        ...
        for entry in conf_text.split("\n"):
            try:
                name, values = DoxygenConfiguration._parse_entry(entry=entry, sep=sep)
            ...
            if name == "INPUT":
                paths = DoxygenConfiguration._convert_to_paths(values, root_dir)
    ...
    def _parse_entry(*, entry: str, sep: str) -> tuple[str, list[str]]:
        ...
        if match is None:
            logger.warning("Did not match an expected Doxygen conf line: %r", entry)
            raise EntryParsingError

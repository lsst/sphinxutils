...
def discover_package_doc_dir(initial_dir: str | pathlib.Path) -> str:
...
def discover_conf_py_directory(initial_dir: str | pathlib.Path) -> str:
...
def _has_conf_py(root_dir: pathlib.Path) -> bool:
...
def _search_parents(initial_dir: pathlib.Path) -> pathlib.Path:
    ...
    msg = (
        f"Cannot detect a conf.py Sphinx configuration file from {initial_dir}. "

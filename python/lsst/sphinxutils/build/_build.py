...
def build_stack_docs(
    root_project_dir: Path | str,
    skipped_names: list[str] | None = None,
    skippedNames: list[str] | None = None,
    doxygen_conf_defaults_path: Path | None = None,
    ...
    select_doxygen_packages: list[str] | None = None,
    skip_doxygen_packages: list[str] | None = None,
    ...
    for package_name, package_info in set_up_packages.items():
        try:
            package_docs = find_package_docs(package_info["dir"], skipped_names=skipped_names)
            packages[package_name] = package_docs
        except NoPackageDocsError as e:
            logger.debug(
    ...
    if enable_package_links:
        ...
        if os.path.isdir(root_static_dir):
            ...
            logger.info(f"Creating _static/ at {root_static_dir}")
    ...
    if enable_doxygen_conf:
        ...
        if select_doxygen_packages is not None and len(select_doxygen_packages) > 0:
            doxygen_packages.intersection_update(select_doxygen_packages)
        if skip_doxygen_packages is not None and len(skip_doxygen_packages) > 0:
        ...
        for package_name in doxygen_packages:
            ...
            if package.doxygen_conf_path and not prefer_doxygen_conf_in:
                ...
                preprocess_package_doxygen_conf(conf=package_doxygen_conf, package=package)
...
def link_directories(root_dir: str, package_doc_dirs: dict[str, Path]) -> None:
    ...
    for dirname, source_dirname in package_doc_dirs.items():
        ...
        message = f"Linking {link_name} -> {source_dirname}"
        logger.info(message)


def remove_existing_links(root_dir: str) -> None:
    ...
    for name in os.listdir(root_dir):
        ...
        if os.path.islink(full_name):
            logger.debug(f"Deleting existing symlink {full_name}")
            os.remove(full_name)

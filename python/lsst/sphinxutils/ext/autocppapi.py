...
def cache_doxylink_symbolmap(
    ...
    try:
        symbol_map: doxylink.SymbolMap | None = load_symbolmap(doxylink_role, config)
...
def load_symbolmap(doxylink_role: str, config: sphinx.config.Config) -> doxylink.SymbolMap:
        ...
        if isinstance(config["doxylink"], dict):
            if isinstance(config["doxylink"][doxylink_role], tuple):
                ...
                if not tag_path.exists():
                    raise SymbolMapLoadError(f"Could not load tag file for Doxylink {doxylink_role} role.")
...
class AutoCppApi(SphinxDirective):
    ...
    def run(self) -> list[nodes.Node]:
        ...
        try:
            key = "documenteer_autocppapi_symbolmaps"
            symbol_map: doxylink.SymbolMap | None = self.env.config[key][doxylink_role]
        except KeyError:
        ...
        if symbol_map is not None:
            ...
            node_list.extend(self._make_fallback_section(prefix=namespace_prefix, heading=namespace_prefix))
    ...
    def _make_namespace_section(
        ...
        *,
        ...
        node_list.extend(self.parse_text_to_nodes("\n".join(rst_text)))
    ...
    def _make_fallback_section(self, *, prefix: str, heading: str) -> list[nodes.Node]:
        ...
        node_list.append(
            nodes.paragraph(text="This section is empty because the Doxygen tag file is not available.")
        )

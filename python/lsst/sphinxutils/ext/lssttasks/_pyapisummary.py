...
from sphinx.addnodes import desc, desc_addname, desc_annotation, desc_content, desc_signature, seealso

try:
    from sphinx.domains.python._annotations import _pseudo_parse_arglist
except ImportError:
    # For compatibility with Sphinx < 7.0
    from sphinx.domains.python import _pseudo_parse_arglist  # type: ignore[attr-defined,no-redef]
...
class TaskApiDirective(SphinxDirective):
    ...
    def _format_summary_node(self, task_class: type) -> list[nodes.Node]:
        ...
        for method in methods:
            if hasattr(task_class, method):
                ...
                nodes.append(self._format_method_nodes(method_obj, modulename, classname))
        return nodes

    def _format_class_nodes(self, task_class: type) -> nodes.Node:
        ...
        desc_sig_node = self._format_signature(signature, modulename, classname, fullname, "py:class")

        # The content is the one-sentence summary.
        content_node = desc_content()
        content_node += self._create_doc_summary(task_class, fullname, "py:class")
    ...
    def _format_method_nodes(self, task_method: type, modulename: str, classname: str) -> nodes.Node:
        ...
        desc_sig_node = self._format_signature(signature, modulename, classname, fullname, "py:meth")

        # The content is the one-sentence summary.
        content_node = desc_content()
        content_node += self._create_doc_summary(task_method, fullname, "py:meth")
    ...
    def _format_signature(
        self, signature: Signature, modulename: str, classname: str, fullname: str, refrole: str
    ) -> nodes.Node:
        xref = PyXRefRole()

        arglist = (
            stringify_signature(signature, show_annotation=False, show_return_annotation=False)
            .lstrip("(")
            .rstrip(")")
        )
    ...
    def _create_doc_summary(self, obj: type, fullname: str, refrole: str) -> nodes.paragraph:
    ...
    def _create_api_details_link(self, fullname: str, refrole: str) -> list[nodes.Node]:
        """Append a link to the API docs, labelled as "...", that is appended
        ...
        ref_text = f"... <{fullname}>"
        xref = PyXRefRole()
        xref_nodes, _ = xref(refrole, ref_text, ref_text, self.lineno, self.state.inliner)
        return xref_nodes
    ...
    def _format_config_nodes(self, modulename: str, classname: str) -> nodes.Node:
        ...
        config attribute.
        """
        fullname = f"{modulename}.{classname}.config"
    ...
    def _format_import_example(self, task_class: type) -> list[nodes.Node]:
        ...
        """
        code = f"from {task_class.__module__} import {task_class.__name__}"
    ...
    def _format_api_docs_link_message(self, task_class: type) -> list[nodes.Node]:
        ...
        """
        fullname = f"{task_class.__module__}.{task_class.__name__}"

        p_node = nodes.paragraph()

        p_node += nodes.Text("See the ")
        ...
        p_node += nodes.Text(" API reference for complete details.")

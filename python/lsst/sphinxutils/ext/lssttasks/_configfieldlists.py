...
from ..utils import make_section
from ._crossrefs import format_configfield_id, pending_config_xref, pending_task_xref
from ._utils import get_subtask_fields, get_task_config_class, get_task_config_fields, get_type, typestring
...
class ConfigFieldListingDirective(SphinxDirective):
    ...
    def run(self) -> list[nodes.Node]:
        ...
        for field_name, field in config_fields.items():
            ...
            field_id = format_configfield_id(
                ".".join((task_config_class.__module__, task_config_class.__name__)),
                field_name,
            )
            ...
            try:
                ...
                logger.debug("Skipping unknown config field type, %r", field)
                continue

            all_nodes.append(format_field_nodes(field_name, field, field_id, self.state, self.lineno))
...
class SubtaskListingDirective(SphinxDirective):
...
class StandaloneConfigFieldsDirective(SphinxDirective):
...
def get_field_formatter(field: Field) -> Callable:
...
def register_formatter(field_typestr: str) -> Callable:
    ...
    def decorator_register(formatter: Callable) -> Callable:
        @functools.wraps(formatter)
        def wrapped_formatter(*args: Any, **kwargs: Any) -> nodes.section:
            field_name = args[0]
            field = args[1]
            field_id = args[2]
            ...
            if not isinstance(field, field_type):
                raise ValueError(
                    f"Field {field_name} ({field!r}) is not an {field_typestr} "
                    f"type. It is an {typestring(field)}."
                )
            ...
            section = make_section(section_id=field_id + "-section", contents=nodes)
            return section
...
def create_python_xref_for_type(
    obj_type: type, calling_directive: SphinxDirective, hide_namespace: bool = False
) -> list[nodes.Node]:
    """Create Python cross-reference docutils nodes for a type.
    ...
    -------
    str
        Source string for a Python cross-reference.
    """
    if obj_type.__module__ == "builtins":
        type_name = obj_type.__name__
    else:
        type_name = ".".join((obj_type.__module__, obj_type.__name__))

    if hide_namespace:
        template = ":py:obj:`~{}`\n"
    else:
        template = ":py:obj:`{}`\n"
    nodes, _ = calling_directive.parse_inline(template.format(type_name))
    return nodes


@register_formatter("lsst.pex.config.config.Field")
def format_field_nodes(
    field_name: str, field: Field, field_id: str, caller: SphinxDirective
) -> list[nodes.Node]:
    ...
    -------
    list
        List of nodes documenting the Field.
    """
    ...
    field_type_item_content_p += create_python_xref_for_type(field.dtype, caller, hide_namespace=False)[
        0
    ].children[0]
    field_type_item_content_p += nodes.Text(" ")
    field_type_item_content_p += create_python_xref_for_type(type(field), caller, hide_namespace=True)[
        0
    ].children[0]
    if field.optional:
        field_type_item_content_p += nodes.Text(" (optional)")
    ...
    dl += create_default_item_node(field)
    dl += field_type_item

    # Doc for this ConfigurableField, parsed as rst
    desc_node = create_description_node(field, caller)

    # Title for configuration field
    title = create_title_node(field_name, field_id, caller)
...
def format_configurablefield_nodes(
    field_name: str, field: ConfigurableField, field_id: str, caller: SphinxDirective
) -> list[nodes.Node]:
    ...
    -------
    list
        List of nodes for the ConfigurableField.
    """
    ...
    dl = nodes.definition_list()
    dl += default_item
    dl += create_field_type_item_node(field, caller)
...
def format_listfield_nodes(
    field_name: str, field: ListField, field_id: str, caller: SphinxDirective
) -> list[nodes.Node]:
    ...
    -------
    list
        List of nodes for the ListField.
    """
    ...
    itemtype_def += create_python_xref_for_type(field.itemtype, caller, hide_namespace=False)
    ...
    field_type_item_content = nodes.definition()
    field_type_item_content_p = nodes.paragraph()
    field_type_item_content_p += create_python_xref_for_type(field.itemtype, caller, hide_namespace=False)[
        0
    ].children[0]
    field_type_item_content_p += nodes.Text(" ")
    field_type_item_content_p += create_python_xref_for_type(type(field), caller, hide_namespace=True)[
    ...
    # Reference target
    ref_target = create_configfield_ref_target_node(field_id, caller.env, caller.lineno)
...
def format_choicefield_nodes(
    field_name: str, field: ChoiceField, field_id: str, caller: SphinxDirective
) -> list[nodes.Node]:
    ...
    -------
    list
        List of documentation nodes for the ChoiceField.
    """
...
def format_rangefield_nodes(
    field_name: str, field: RangeField, field_id: str, caller: SphinxDirective
) -> list[nodes.Node]:
    ...
    -------
    list
        List of nodes documenting the RangeField.
    """
    # Field type
    field_type_item = nodes.definition_list_item()
    field_type_item.append(nodes.term(text="Field type"))
...
def format_dictfield_nodes(
    field_name: str, field: DictField, field_id: str, caller: SphinxDirective
) -> list[nodes.Node]:
    ...
    -------
    list
        Nodes documenting the DictField.
    """
    # Custom value type field for definition list
    valuetype_item = nodes.term(text="Value type")
    valuetype_def = nodes.definition()
    valuetype_def += create_python_xref_for_type(field.itemtype, caller, hide_namespace=False)
    ...
    dl += create_default_item_node(field)
    dl += create_field_type_item_node(field, caller)
    dl += create_keytype_item_node(field, caller)
    dl += valuetype_item
...
def format_configfield_nodes(
    field_name: str, field: ConfigField, field_id: str, caller: SphinxDirective
) -> list[nodes.Node]:
    ...
    -------
    list
        Nodes documenting the ConfigField.
    """
...
def format_configchoicefield_nodes(
    field_name: str, field: ConfigChoiceField, field_id: str, caller: SphinxDirective
) -> list[nodes.Node]:
    ...
    -------
    list
        Nodes documenting the ConfigChoiceField.
    """
    # Create a definition list for the choices
    choice_dl = nodes.definition_list()
    ...
    field_type_item_content_p += nodes.Text(multi_text)
    field_type_item_content_p += create_python_xref_for_type(type(field), caller, hide_namespace=True)[
        0
    ].children[0]
    if field.optional:
...
def format_configdictfield_nodes(
    field_name: str, field: ConfigDictField, field_id: str, caller: SphinxDirective
) -> list[nodes.Node]:
    ...
    -------
    list
        Nodes documenting the ConfigDictField.
    """
...
def format_registryfield_nodes(
    field_name: str, field: RegistryField, field_id: str, caller: SphinxDirective
) -> list[nodes.Node]:
    ...
    -------
    list
        Nodes documenting the RegistryField.
    """
    ...
    for choice_value, choice_class in field.registry.items():
        ...
        if hasattr(choice_class, "__module__") and hasattr(choice_class, "__name__"):
            name = ".".join((choice_class.__module__, choice_class.__name__))
...
def create_field_type_item_node(field: Field, directive: SphinxDirective) -> nodes.definition_list_item:
    ...
    type_item_content_p += create_python_xref_for_type(type(field), directive, hide_namespace=True)[
        0
    ].children
    if field.optional:
        type_item_content_p += nodes.Text(" (optional)")
    type_item_content += type_item_content_p
    type_item += type_item_content
    return type_item


def create_default_item_node(field: Field) -> nodes.definition_list_item:
...
def create_keytype_item_node(field: DictField, directive: SphinxDirective) -> nodes.definition_list_item:
    ...
    keytype_def += create_python_xref_for_type(field.keytype, directive, hide_namespace=False)
    keytype_node += keytype_def
    return keytype_node


def create_description_node(field: Field, directive: SphinxDirective) -> nodes.Node:
    """Create docutils nodes for the Field's description, built from the
    ...
    new_nodes, _ = directive.parse_inline(field.doc, lineno=directive.lineno)
    doc_container_node += new_nodes

    return doc_container_node


def create_title_node(field_name: str, field_id: str, directive: SphinxDirective) -> nodes.title:
    ...
    # Reference target
    env = directive.env
    ref_target = create_configfield_ref_target_node(field_id, env, directive.lineno)
...
def create_configfield_ref_target_node(target_id: str, env: BuildEnvironment, lineno: int) -> nodes.target:
    ...
    docutils.nodes.target
        Target node for the configuration field.
    ...
    if not hasattr(env, "lsst_configfields"):
        setattr(env, "lsst_configfields", {})
    getattr(env, "lsst_configfields")[target_id] = {
        "docname": env.docname,
        "lineno": lineno,
        "target": target_node,
    }

...
class BaseTopicListDirective(SphinxDirective):
    ...
    def _build_toctree(self) -> sphinx.addnodes.toctree:
        ...
        docnames = [docname for docname in self._env.found_docs if docname.startswith(root)]
...
class task_topic_list(nodes.comment):  # noqa: N801
...
def process_task_topic_list(app: Sphinx, doctree: nodes.Node, fromdocname: str) -> None:
    ...
    for node in doctree.traverse(task_topic_list):
        topics = getattr(env, "lsst_task_topics", None)
        if topics is None:
            message = "Environment does not have 'lsst_task_topics', can't process the listing."
        ...
        for key in topic_keys:
            ...
            ref_node["refuri"] = app.builder.get_relative_uri(fromdocname, topic["docname"])
            # NOTE: Not appending an anchor to the URI because task topics
            # are designed to occupy an entire page.
            link_label = nodes.Text(class_name)
            ref_node += link_label
